import { VenmoResponseSchema } from "../types/schemas";
import type { VenmoRequestPayload } from "../types/venmo";
import { logger } from "../lib/logger";

/**
 * Service for Venmo API integration
 */
export class VenmoService {
  ...
  async sendPaymentRequest(recipientUserId: string, amount: number, note: string): Promise<string> {
    logger.info(`Sending Venmo request for $${amount.toFixed(2)}...`);

    if (this.dryRun) {
      const mockTransactionId = `mock_${Date.now()}`;
      logger.info(`[DRY RUN] Venmo request would be sent: $${amount.toFixed(2)} (Mock ID: ${mockTransactionId})`);
    ...
    try {
      ...
      logger.info(`Venmo request sent: $${amount.toFixed(2)} to ${recipientUserId} (Request ID: ${transactionId})`);
